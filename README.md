# Veryl TAP Controller

This repo contains a minimal TAP Controller implemented in Veryl.
The bundled `./src/fpga_top.veryl` example, OpenOCD config, Makefile programming flow and constraints assume a Numato Lab Mimas ECP-5 Mini board, but the TAP controller implementation itself should be platform agnostic (only the pins must be remapped).
